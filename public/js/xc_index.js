$(function () {
    ...
    // 滑动到nav-header  顶部导航变为固定定位
    // var navTop = $(".logo_container").offset().top;
    // $(window).scroll(function () {
    //     if ($(document).scrollTop() >= navTop) {
    //         // 变为固定定位
    //         $('.top').css({ 'position': 'fixed','left':169, 'top': 0, 'zIndex': 200,'opacity':0.95,'borderBottomLeftRadius':4,'borderBottoRightRadius':4})
    //     } else {
    //         $('.top').css({'position': 'absolute' ,'borderBottomLeftRadius':0,'borderBottoRightRadius':0})
    //     }
    // });
})